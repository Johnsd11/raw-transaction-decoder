...
fn read_u64(transaction_bytes: &mut &[u8]) -> Amount {
    let mut buffer = [0; 8];
    transaction_bytes.read(&mut buffer).unwrap();
...
fn main() {
    ...
    for _ in 0..input_count {
        ...
        inputs.push(Input {
            // can also just get rid of key pairs because they are the same name
            txid,
            output_index,
            script_sig,
            sequence
    ...
    for _ in 0..output_count {
        let amount = read_u64(&mut bytes_slice).to_btc();
